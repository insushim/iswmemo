...
import { View, Text, StyleSheet, TouchableOpacity, RefreshControl, TextInput, Modal, Alert, Animated } from 'react-native';
...
export default function GoalsScreen() {
  ...
  const fetchGoals = async () => {
    try {
      ...
      setGoals(data || []);
      SecureStore.setItemAsync(GOALS_CACHE_KEY, JSON.stringify(data || [])).catch(() => {});
    }
    catch (error) { console.error('Goals fetch error:', error); }
  };

  useFocusEffect(useCallback(() => { if (!hasLoadedRef.current) { hasLoadedRef.current = true; fetchGoals(); } }, []));
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing && editId) {
        await api.updateGoal(editId, { title, type: newGoalType, color: selectedColor });
      } else {
        const created = await api.createGoal({ title, type: newGoalType, color: selectedColor, status: 'IN_PROGRESS' }) as any;
  ...
  const renderRightActions = (goal: Goal) => (progress: Animated.AnimatedInterpolation<number>, dragX: Animated.AnimatedInterpolation<number>) => {
    ...
    return (
      <Animated.View style={[styles.swipeDelete, { transform: [{ scale }] }]}>
        <TouchableOpacity style={styles.swipeDeleteBtn} onPress={() => handleDeleteGoal(goal)}>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <DraggableFlatList
        ...
        renderItem={({ item: goal, drag, isActive }: RenderItemParams<Goal>) => {
          ...
          return (
            <ScaleDecorator>
              ...
              >
                <TouchableOpacity
                  activeOpacity={0.7}
                  style={[styles.goalItem, { backgroundColor: colors.card, borderWidth: isPinned ? 1.5 : 0, borderColor: isPinned ? colors.primary : 'transparent', padding: cardPadding, opacity: isActive ? 0.8 : 1 }]}
                ...
                >
                  <View style={styles.goalTop}>
                    ...
                    >
                      <Pin size={14} color={isPinned ? colors.primary : colors.mutedForeground} fill={isPinned ? colors.primary : 'none'} />
                    </TouchableOpacity>
                  </View>
                </TouchableOpacity>
              </Swipeable>
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <TextInput
                ...
                blurOnSubmit={false}
              />
              <VoiceInput color={colors.primary} onResult={(text) => setNewGoalTitle(prev => prev ? prev + ' ' + text : text)} />
            </View>
