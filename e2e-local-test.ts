...
async function runFullTest() {
  ...
  console.log('GrowthPad 전체 기능 테스트 (로컬 3001)');
  console.log('='.repeat(60));
  ...
  try {
    ...
    // 할일 추가 버튼 클릭
    const addBtn = page.locator('button:has-text("할 일 추가")').first();
    await addBtn.click();
  ...
  try {
    ...
    // 새 메모 버튼 클릭 (첫 번째 버튼 사용)
    const addNoteBtn = page.locator('button:has-text("새 메모")').first();
